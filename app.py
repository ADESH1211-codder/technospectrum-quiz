...
<a href="/admin" class="btn btn-sm btn-dark">Admin Home</a>
</div>
"""
    return f"""
<!doctype html>
...
def leaderboard():
    ...
    for i,r in enumerate(rows):
        t+=f"<tr><td>{i+1}</td><td>{r[1]}</td><td>{r[2]}</td><td>{round(r[3],1)} s</td></tr>"
...
def export():
    wb=Workbook()
    ws=wb.active
    ws.append(["Rank","Name","Email","Contact","Score","Time(s)"])
    for c in ws[1]:
